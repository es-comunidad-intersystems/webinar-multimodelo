...
public class InterSystemsIRISDialect extends Dialect {
    ...
    private static final AbstractLimitHandler LIMIT_HANDLER = new AbstractLimitHandler() {
        ...
        public String processSql(String sql, RowSelection selection) {
            if (LimitHelper.useLimit(this, selection)) {
                ...
                if (selection != null && selection.getFirstRow() != null && selection.getFirstRow() != null) {
                    int offset = selection.getFirstRow() + 1;
                    int limit = selection.getFirstRow() + selection.getMaxRows();

                    // NOTE: The following line replaces the initial 'select' statement with one
                    // that contains 'TOP ALL' that is needed for ordering/sorting.
                    sql = "select * from (" + "select TOP ALL " + sql.substring(7) + ") where %vid between (" + offset
                            + ") and (" + limit + ")";
                }
    ...
    protected final void commonRegistration() {
        ...
        registerFunction("current_timestamp",
                new ConditionalParenthesisFunction("current_timestamp", StandardBasicTypes.TIMESTAMP));
        ...
        // is it necessary to register %exact since it can only appear in a where
        // clause?
        ...
        // use Hibernate implementation "From" is one of the parameters they pass in
        // position ?3
    ...
    public String getAddForeignKeyConstraintString(String constraintName, String[] foreignKey, String referencedTable,
        ...
        return new StringBuilder(300).append(" ADD CONSTRAINT ").append(constraintName).append(" FOREIGN KEY ")
                .append(constraintName).append(" (")
                .append(StringHelper.join(", ", Arrays.stream(foreignKey).iterator())).append(") REFERENCES ")
                .append(referencedTable).append(" (")
                .append(StringHelper.join(", ", Arrays.stream(primaryKey).iterator())).append(") ").toString();
    }
    ...
    /*
     * @Override public Class getNativeIdentifierGeneratorStrategy() { return
    ...
    // It really does support sequences, but InterSystems elects to suggest usage of
    // IDENTITY instead :/
    // Anyway, below are the actual support overrides for users wanting to use this
    // combo...
    //
    // public String getSequenceNextValString(String sequenceName) {
    // return "select InterSystems.Sequences_GetNext('" + sequenceName + "') from
    // InterSystems.Sequences where ucase(name)=ucase('" + sequenceName + "')";
    // }
    //
    ...
    public LockingStrategy getLockingStrategy(Lockable lockable, LockMode lockMode) {
        ...
        // Just to make some tests happy, but InterSystems IRIS doesn't really support
        // this.
        ...
        // InterSystems InterSystemsIRIS does not current support "SELECT ... FOR
        // UPDATE" syntax...
    ...
    public boolean bindLimitParametersFirst() {
        // Does the LIMIT clause come at the start of the SELECT statement, rather than
        // at the end?
        return true;
    }
    ...
    @SuppressWarnings("deprecation")
    public boolean useMaxForLimit() {
        // Does the LIMIT clause take a "maximum" row number instead of a total number
        // of returned rows?
        return true;
    }
     ...
     * @Override
     * 
     * @SuppressWarnings("deprecation") public String getLimitString(String sql,
     * boolean hasOffset) { if ( hasOffset ) { throw new
     * UnsupportedOperationException( "query result offset is not supported" ); }
     * 
     * 
     * }
     */
    ...
    // if ( hasOffset ) {
    // throw new UnsupportedOperationException( "query result offset is not
    // supported" );
    // }
    //
    ...
    public String getLimitString(String sql, int offset, int limit) {
        ...
        return "select * from (" + sql + ") where %vid between " + offset + " and (" + limit + " + " + offset + " - 1)";
        /*
         * DB2 IMPLEMENTATION if ( offset == 0 ) {
         * 
         * return sql + " fetch first " + limit + " rows only"; } //nest the main query
         * in an outer select return
         ...
         * final int insertionPoint = sql.startsWith( "select distinct" ) ? 15 : 6;
         * 
         * String modifiedSql = StringBuilder( sql.length() + 8 ) .append( sql )
         * .insert( insertionPoint, " TOP ? " ) .toString();
         */
    ...
    /**
     * ddl like ""value" integer null check ("value">=2 AND "value"<=10)" isn't
     * supported
     */
    @Override
    ...
    /**
     * select count(distinct a,b,c) from hasi isn't supported ;)
     */
    @Override
